...
function createReadyPromise(details) {
  ...
  requestAnimationFrame(() => {
    ...
    if (details.pendingTask === 'play') {
      isReady = timelineTime !== null && (details.startTime !== null || details.holdTime !== null)
    } else {
      isReady = timelineTime !== null
    }
    if (isReady)
      notifyReady(details);
  });
...
function tickAnimation(timelineTime) {
  ...
  if (details.pendingTask && (details.startTime !== null || details.holdTime !== null)) {
    notifyReady(details);
  }
...
export class ProxyAnimation {
  ...
  set currentTime(value) {
    ...
    //    5. Resolve animation’s current ready promise with animation.
    if (details.pendingTask == 'pause') {
      details.holdTime = fromCssNumberish(value);
      applyPendingPlaybackRate(details);
      details.startTime = null;
      details.pendingTask = null;
      details.readyPromise.resolve(this);
